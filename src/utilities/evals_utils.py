...
def get_transition_matrix(env, model): #CHECKED
    ...
    for state_index in range(state_space_size):
        ...
        if CONFIG['CNN_preprocessing']:
            with torch.no_grad():
                obs_tensor = pre_processor(torch.Tensor(obs_tensor).view(1, 2 * 5, 5, 5))
        with torch.no_grad():
            _, _, _, _ = model.get_action_and_value(obs_tensor)
            logits = model.actor(obs_tensor.reshape(obs_tensor.size(0), -1))
